        ...
        if ( current_user_can( 'manage_options' ) && !get_user_meta( $wcz_user_id, 'wcz_feedback_notice_dismiss' ) ) {
            ...
            ?>
					<p class="wcz-admin-special">
						<?php 
            esc_html_e( 'Use the coupon "30SALE22" to get 30% off any StoreCustomizer license, also applies to future renewals!', 'woocustomizer' );
            ?>
					</p>
				<?php 
            ?>
    ...
    public function wcz_quicknote_notice()
    {
        ...
        if ( current_user_can( 'manage_options' ) && !get_user_meta( $wcz_user_id, 'wcz_quicknote_notice_dismiss_2' ) && 'admin.php?page=wcz_settings&' == $wczpage ) {
            ...
            ?>
				...
				<p>
					<?php 
            esc_html_e( 'We\'re running a limited 30% special on StoreCustomizer for all licenses (applies to first payments and renewals, forever).', 'woocustomizer' );
            ?>
				</p>
				<p class="wcz-admin-special">
					<?php 
            esc_html_e( 'Use the coupon "30SALE22" to get your discount now!', 'woocustomizer' );
            ?>
				</p>
				<a href="<?php 
