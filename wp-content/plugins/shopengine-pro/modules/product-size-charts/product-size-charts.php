...
class Product_Size_Charts {
	...
	<div id="shopengine_product_size_charts" class="panel woocommerce_options_panel">
		<div class="options_group shopengine_product_size_charts-content care_instruction">
			...
			?>
			<p><?php esc_html_e('Click on this link to create a new chart', 'shopengine-pro')?> <a href="<?php echo get_admin_url() . 'edit.php?post_type=shopengine-template#shopengine-modules' ?>"><?php esc_html_e('Link', 'shopengine-pro')?></a></p>
		</div>
	</div>
	<?php
	}
	...
	public function save_tab_data($product_id) {
		...
		if (isset($_POST[self::OPTION_STATUS_KEY])) {
			$status = sanitize_text_field($_POST[self::OPTION_STATUS_KEY]);
		} else {
			$status = 'no';
		}
		...
		if (isset($_POST[self::OPTION_KEY])) {
			update_post_meta(
				$product_id,
				self::OPTION_KEY,
				sanitize_text_field($_POST[self::OPTION_KEY])
			);
		}
	}
}