...
class Back_Order {
	...
	public function variation_settings_fields($loop, $variation_data, $variation) {
		...
		echo '<div id="options_group_variation' . $loop . '">';
	...
	public function save_variation_settings_fields($variation_id, $loop) {
		...
		$dt = empty($_POST[$this->field_names['variation_avl_date']][$loop]) ? '' :
			date('Y-m-d', strtotime($_POST[$this->field_names['variation_avl_date']][$loop]));
		 ...
		 * Positive (0<=) : :D
		 */
		$max = strlen(trim($_POST[$this->field_names['variation_max_limit']][$loop])) <= 0 ? '' : intval($_POST[$this->field_names['variation_max_limit']][$loop]);
	...
	public function set_column_val_in_order_listing_page($column) {
		...
		if($column == 'order_page_order_type') {
			...
			if($is_backordered) {
				...
				echo '<strong class="shopengine-partial-payment-product-badge">' . esc_html__('On Back-Order', 'shopengine-pro') . '</strong>';
				echo \Shopengine\Utils\Helper::kses($msg);
			}
		}
	}
	...
	public function save_custom_backorder_fields($product_id, $post) {
		...
		$dt = empty($_POST['_backorder_available_date']) ? '' : date('Y-m-d', strtotime($_POST['_backorder_available_date']));
		 ...
		 * Positive (0<=) : :D
		 */
		$max = strlen(trim($_POST['_backorder_max_limit'])) <= 0 ? '' : intval($_POST['_backorder_max_limit']);
	...
	function get_orders_by_product_id($product_id, $exclude = false, $exclude_status = ['wc-completed']) {
		...
		if($exclude === true) {
			...
			$qry = "SELECT order_items.order_id
        FROM {$wpdb->prefix}woocommerce_order_items as order_items
        LEFT JOIN {$wpdb->prefix}woocommerce_order_itemmeta as order_item_meta ON order_items.order_item_id = order_item_meta.order_item_id
        LEFT JOIN {$wpdb->posts} AS posts ON order_items.order_id = posts.ID
        WHERE posts.post_type = 'shop_order'
        AND posts.post_status NOT IN ( '" . implode("','", $exclude_status) . "' )
        AND order_items.order_item_type = 'line_item'
        AND order_item_meta.meta_key = '_product_id'
        AND order_item_meta.meta_value = '$product_id'";
        ...
        LEFT JOIN {$wpdb->posts} AS posts ON order_items.order_id = posts.ID
        WHERE posts.post_type = 'shop_order'
        AND posts.post_status IN ( '" . implode("','", $exclude_status) . "' )
        AND order_items.order_item_type = 'line_item'
        AND order_item_meta.meta_key = '_product_id'
        AND order_item_meta.meta_value = '$product_id'";
		}
	...
	function get_order_item_metas_by_orders($order_ids = [], $with_like = false, $like_val = '') {
		...
		WHERE ord_item.order_id IN(" . implode("','", $order_ids) . ") 
		";
		...
		if($with_like === true) {
			...
			LEFT JOIN {$wpdb->prefix}woocommerce_order_items AS ord_item ON ord_item.order_item_id = ord_item_meta.order_item_id 
			WHERE ord_item.order_id IN(" . implode(",", $order_ids) . ") 
			AND ord_item_meta.meta_key LIKE '{$like_val}'
			";
