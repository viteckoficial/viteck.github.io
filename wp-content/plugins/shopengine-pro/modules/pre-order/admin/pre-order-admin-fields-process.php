...
class Pre_Order_Admin_Fields_Process {
	...
	public function change_sale_price_from_pre_order_price( $product_id, $post ) {
		...
		} else if ( isset( $_POST['shopengine_pre_order_price'] ) && $_POST['shopengine_pre_order_price'] ) {
	...
	public function change_variant_sale_price_from_pre_order_price( $variation_id, $loop ) {
		...
		$pre_order_price = $_POST['shopengine_pre_order_price'][ $loop ] ?? 0;
	...
	public function save_variation_settings_fields( $variation_id, $loop ) {
		...
		if ( isset( $_POST['variable_stock_status'][ $loop ] ) && ( 'pre_order' == $_POST['variable_stock_status'][ $loop ] ) ) {
			...
			$max_order = $_POST['shopengine_pre_order_max_order'][ $loop ] ?? null;
			...
			$available_date = $_POST['shopengine_pre_order_available_date'][ $loop ] ?? null;
			...
			$price = $_POST['shopengine_pre_order_price'][ $loop ] ?? null;
			...
			if ( isset( $_POST['shopengine_pre_order_product_message'][ $loop ] ) ) {
				update_post_meta(
					...
					$_POST['shopengine_pre_order_product_message'][ $loop ]
				);
			}

			$auto_convert = $_POST['shopengine_pre_order_auto_convert'][ $loop ] ?? null;
	...
	public function save_partial_payment_product_meta( $post_id ) {
		...
		if ( isset( $_POST['_stock_status'] ) && ( 'pre_order' == $_POST['_stock_status'] ) && !isset( $_POST['variable_stock_status'] ) ) {
			...
			$max_order = $_POST['shopengine_pre_order_max_order'] ?? null;
			if ( $max_order && (int) $max_order ) {
			...
			$available_date = $_POST['shopengine_pre_order_available_date'] ?? null;
			if ( $available_date && $this->validateDate( $available_date ) ) {
			...
			$price = $_POST['shopengine_pre_order_price'] ?? null;
			...
			$message = $_POST['shopengine_pre_order_product_message'] ?? null;
			if ( $message ) {
				update_post_meta(
					$post_id,
					'shopengine_pre_order_product_message',
			...
			$auto_convert = $_POST['shopengine_pre_order_auto_convert'] ?? null;
			if ( $auto_convert ) {
				update_post_meta(
					$post_id,
					'shopengine_pre_order_auto_convert',
