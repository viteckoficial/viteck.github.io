...
class Backend {
    ...
    private function order_details($order, $type) {
        if (isset($_GET['post_type']) && $_GET['post_type'] === 'shop_order') {
            $fields = $this->settings[$type]['value'];
            $order_id = 0;
        } else {
        ...
        <br class="clear" />
        <?php if ($type === 'billing'): ?>
		    <h4><?php esc_attr_e('Additional billing fields created from the shopengine', 'shopengine-pro')?><a href="#" class="edit_address"></a></h4>
        <?php else: ?>
		<h4><?php esc_attr_e('Additional shipping fields created from the shopengine', 'shopengine-pro')?><a href="#" class="edit_address"></a></h4>
        <?php endif;?>
		<div class="address">
	    <?php
foreach ($fields as $value) {
            $name = Checkout_Additional_Field::make_useable_input_name($value['name']);
            echo "<p><strong>{$value['label']}</strong>" . get_post_meta($order_id, $name, true) . "</p>";
        }
        ?>
		</div>
