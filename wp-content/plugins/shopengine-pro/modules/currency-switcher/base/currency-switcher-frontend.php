...
class Currency_Switcher_Frontend {
	...
	public function hooks() {
		...
		add_filter('woocommerce_variation_prices', [$this, 'variation_product_reqular_price'], 9999);
	...
	public function variation_product_reqular_price($prices) {
		$updated_price = [];
	...
	public function payment_gateways($gateways) {
		...
		if (empty($_SERVER['HTTP_REFERER']) || strpos($_SERVER['HTTP_REFERER'], 'shopengine-template') === false) {
	...
	public function find_currency($settings) {
		...
		if (!empty($_GET['currency'])) {
			$codes[] = sanitize_text_field($_GET['currency']);
		} 
